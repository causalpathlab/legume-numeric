...
fn gene_slices_tile_the_axis_exactly() {
    let slices: Vec<(usize, usize)> = crate::candle::decoder::gene_slices(10, 4).collect();
    assert_eq!(slices, vec![(0, 4), (4, 4), (8, 2)]);
    assert_eq!(
        crate::candle::decoder::gene_slices(0, 4).count(),
        0,
        "empty axis yields none"
    );
    assert_eq!(
        crate::candle::decoder::gene_slices(5, 0).collect::<Vec<_>>(),
