...
//!   trainer driven by [`crate::candle::data::masked_dense::DenseMaskedLevel`].
//!
...
/// in log-space (`log σ` via [`crate::candle::loss::log_sigmoid`] + an inclusive cumsum
...
pub fn stick_breaking_log_simplex(logits_nk: &Tensor) -> candle_core::Result<Tensor> {
    ...
    let log_1mv = crate::candle::loss::log_sigmoid(&eta.neg()?)?; // log(1−v_k) = log σ(−η_k)
