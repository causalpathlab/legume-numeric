...
use crate::matrix::common_io::{read_lines_of_words_delim, ReadLinesOut};
use log::info;
...
impl Membership {
    ...
    pub fn match_keys(&self, query_keys: &[Box<str>]) -> (HashMap<Box<str>, Box<str>>, MatchStats) {
        ...
        // Through `crate::matrix::progress`, per that module's contract: a bar
        // built straight from indicatif misses the shared MultiProgress
        // and corrupts interleaved log output.
        let bar = crate::matrix::progress::new_progress_bar(query_keys.len() as u64)
